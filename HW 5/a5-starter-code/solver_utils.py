...
def value_iteration(
    ...
    max_delta = 0.0
    # *** BEGIN OF YOUR CODE ***
        # Iterate over nonterminal states
    for state in mdp.nonterminal_states:
        ...
        for action in mdp.actions:
            ...
            for next_s in mdp.all_states:
                T = mdp.transition(state, action, next_s)
                R = mdp.reward(state, action, next_s)
                v_next = mdp.config.gamma * v_table[next_s]
                q_value += T * (R + v_next)
            q_values.append(q_value)
            q_table[state, action] = q_value
                
        new_v_table[state] = max(q_values) # Update v table with max Q-value
...
def extract_policy(
    ...
    policy = {}
    for state in mdp.nonterminal_states:
        policy[state] = max(mdp.actions, key=lambda action: q_table.get((state, action), 0.0))
...
def q_update(
    ...
    new_q = max(q_table.get((next_state, i), 0.0) for i in mdp.actions)    
    new_q = (1-alpha)*q_value + alpha*(reward + mdp.config.gamma*new_q)
...
def extract_v_table(mdp: tm.TohMdp, q_table: tm.QTable) -> tm.VTable:
    ...
    for state in mdp.all_states:
        v_table[state] = max(q_table.get((state, i), 0.0) for i in mdp.actions)
    return v_table
...
def choose_next_action(
    ...
    best_actions = []
    for action in mdp.actions:
        if q_table.get((state, action), 0) == max(q_table.get((state, i), 0) for i in mdp.actions):
